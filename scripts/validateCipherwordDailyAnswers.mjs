...
if (answers.length !== expected) throw new Error(`Expected ${expected} answers, found ${answers.length}`);
const seen = new Set(answers);
if (seen.size !== answers.length) throw new Error(`Duplicate answer count: ${answers.length - seen.size}`);
for (const answer of answers) {
  if (!/^[a-z]{4,10}$/.test(answer)) throw new Error(`Invalid answer: ${answer}`);
}
console.log(`Cipherword daily bank valid: ${answers.length} unique answers, 2026-06-01 through 2050-12-31.`);
